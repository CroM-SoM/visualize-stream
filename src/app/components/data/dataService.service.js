    ...
    function dtservice($http) {
        ...
        vm.checkUserData = function(data) {
            for (var i = 0; i < data.length; i++) {
                ...
                // var res = data[i].user.location.slice(0,5);
                var cityvalue = vm.include(vm.citiesList, data[i].user.location);
                var language = vm.include(vm.languageList, data[i].user.lang);
                var timezone = vm.include(vm.timezoneList, data[i].user.time_zone);
                ...
                // Retrieve user data from database if location is not in NL.
                var userHistory = vm.checkTouristOne(data[i]);
        ...
        vm.checkTouristOne = function(userData) {
            ...
            if (userData.tourist.cityvalue != true) {
                return vm.apiMethod('data' + userData.user.id);
